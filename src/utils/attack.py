...
def project(x, original_x, epsilon, _type='linf'):
    """
    Project the perturbation to the epsilon l_p ball around the original data.
    """
    if _type == 'linf':
...
class FastGradientSignUntargeted():
    ...
    """
    def __init__(self, device, model, epsilon, alpha, min_val, max_val, max_iters, _type='linf'):
        self.device = device
        self.model = model
        ...
        with torch.enable_grad():
            for _iter in range(self.max_iters):
                ...
                if reduction4loss == 'none':
                    grad_outputs = torch.ones_like(loss, device=self.device)
                else:
                    grad_outputs = None
